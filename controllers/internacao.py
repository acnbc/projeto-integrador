...
async def get_internacao(skip: int = 0, limit: int = 100, db:Session=Depends(get_db)):
...
async def post_internacao(internacao:schemas.Internacao, db:Session=Depends(get_db)):
...
async def get_internacao(id: int, db:Session=Depends(get_db)):
...
async def put_internacao_alta(id: int, alta_internacao:schemas.AltaInternacao, db:Session=Depends(get_db)):
    internacao = repository.get_internacao(db, id)
    if internacao is None:
...
async def put_internacao(id: int, update_internacao:schemas.Internacao, db:Session=Depends(get_db)):
    internacao = repository.get_internacao(db, id)
    if internacao is None:
        raise HTTPException(status_code=400, detail="Internação não encontrada")
...
async def delete_internacao(id: int, db:Session=Depends(get_db)):
    internacao = repository.get_internacao(db, id)
    if internacao is None:
        raise HTTPException(status_code=400, detail="Internação não encontrada")
